...
func (m *Mail) ChooseAPI(msg Message) error {
	...
	default:
		return fmt.Errorf("unknow api %s; only mailgun, sparkpost, sendgrid supported", m.API)
	}
}
...
func (m *Mail) SendUsingApi(msg Message, transport string) error {
	...
	apidriver, err := apimail.NewClient(transport, cfg)
	if err != nil {
		return err
	}
	...
	// TODO add attachemnts
	err = m.addAPIAttachments(msg, tx)
	if err != nil {
		return err
	}

	_, err = apidriver.Send(tx)
	if err != nil {
		return err
	}
...
func (m *Mail) addAPIAttachments(msg Message, tx *apimail.Transmission) error {
	if len(msg.Attachments) > 0 {
		var attachemnts []apimail.Attachment
		...
		for _, x := range msg.Attachments {
			...
			attachemnts = append(attachemnts, attach)
		}
		tx.Attachments = attachemnts
	}
	return nil
}
