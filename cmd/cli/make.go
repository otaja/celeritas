...
func doMake(arg2, arg3 string) error {
	...
	case "key":
		...
		color.Yellow("32 caracters encryption key: %s", rnd)
	case "migration":
	...
	case "model":
		...
		if plur.IsPlural(arg3) {
			...
			tabelName = strings.ToLower(tabelName)
		} else {
			tabelName = strings.ToLower(plur.Plural(arg3))
		}
		...
		model = strings.ReplaceAll(model, "$TABLENAME$", tabelName)
	...
	case "mail":
		if arg3 == "" {
			exitGracefully(errors.New("you must give this service a name as argument"))
		}

		htmlMail := cel.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
		plainMail := cel.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.tmpl"

		err := copyFilefromTemplate("templates/mail/mail.html.tmpl", htmlMail)
		if err != nil {
			exitGracefully(err)
		}

		err = copyFilefromTemplate("templates/mail/mail.plain.tmpl", plainMail)
		if err != nil {
			exitGracefully(err)
		}
