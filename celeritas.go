...
import (
	...
	"github.com/otaja/celeritas/cache"
	"github.com/otaja/celeritas/mailer"
	"github.com/otaja/celeritas/render"
	"github.com/otaja/celeritas/session"
...
func (c *Celeritas) New(rootPath string) error {
	...
	// create a new Scheduler CRON Job
	scheduler := cron.New()
	c.Scheduler = scheduler

	if os.Getenv("CACHE") == "redis" || os.Getenv("SESSION_TYPE") == "redis" {
		myRedisCache = c.createRedisClientCache()
		c.Cache = myRedisCache
		redisPool = myRedisCache.Conn
	}

	if os.Getenv("CACHE") == "badger" {
		myBadgerCache = c.createBadgerClientCache()
	...
	sess := session.Session{
		...
		CookiePersists: c.config.Cookie.persist,
...
func (c *Celeritas) Init(p initPaths) error {
	...
	for _, path := range p.folderNames {
		// check for folders or create it doesnt exist
...
func (c *Celeritas) ListenAndServe() {
	...
	c.InfoLog.Printf("Listining on port %s", os.Getenv("PORT"))
...
func (c *Celeritas) createMailer() mailer.Mail {
	...
	m := mailer.Mail{
		...
		FromAddress: os.Getenv("FROM_ADRESS"),
		Jobs:        make(chan mailer.Message, 20), // last value set the limit of the channel
		Results:     make(chan mailer.Result, 20),  // last value set the limit of the channel
		API:         os.Getenv("MAILER_API"),
		APIKey:      os.Getenv("MAILER_KEY"),
		APIUrl:      os.Getenv("MAILER__URL"),
	}
	return m
}

func (c *Celeritas) createRedisClientCache() *cache.RedisCache {
...
func (c *Celeritas) createBadgerClientCache() *cache.BadgerCache {
